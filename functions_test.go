...
func TestReadDiscreteInputs(t *testing.T) {
	...
	s.DiscreteInputs[255][0] = 1
	s.DiscreteInputs[255][7] = 1
	s.DiscreteInputs[255][8] = 1
	s.DiscreteInputs[255][9] = 1
...
func TestReadHoldingRegisters(t *testing.T) {
	s := NewServer([]uint8{255})
	s.HoldingRegisters[255][100] = 1
	s.HoldingRegisters[255][101] = 2
	s.HoldingRegisters[255][102] = 65535
...
func TestReadInputRegisters(t *testing.T) {
	s := NewServer([]uint8{255})
	s.InputRegisters[255][200] = 1
	s.InputRegisters[255][201] = 2
	s.InputRegisters[255][202] = 65535
...
func TestWriteHoldingRegisters(t *testing.T) {
	...
	expect := []uint16{3, 4}
	got := s.HoldingRegisters[255][1:3]
	if !isEqual(expect, got) {
		t.Errorf("expected %v, got %v\n", expect, got)
	}
}
